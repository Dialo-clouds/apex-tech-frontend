...
function ProductContent() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      addItem({ id: product.id, name: product.name, price: product.price, image: window.location.origin + product.image });
...
// ✅ Main Export: Wrap ProductContent in Suspense
export default function ProductPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0A0A0A] text-white flex items-center justify-center">Loading product...</div>}>
      <ProductContent />
    </Suspense>
  );
}